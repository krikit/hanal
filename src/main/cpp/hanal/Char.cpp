...
std::vector<std::shared_ptr<Char>> Char::characterize(const char* text) {
  ...
  auto ko_KR_utf8 = std::locale("ko_KR.utf8");
  auto& facet = std::use_facet<std::codecvt<wchar_t, char, std::mbstate_t>>(ko_KR_utf8);
  ...
  for (const char* from_curr = text; *from_curr != '\0'; from_curr = from_next) {
    ...
    HANAL_ASSERT(result == std::codecvt_base::partial,
