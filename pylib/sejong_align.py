...
__date__ = 'creation: 2014-06-17, modification: 2014-06-17'
...
def proc_sandwich(forward_pairs, uni_word, morphs, backward_pairs):
  """
  ...
  if uni_word and morphs:
    ...
    if word_morphs_str in _CORRECT_ALIGNS:
      ...
      del morphs[:]
    elif word_morphs_str in ['해야 하/VV + 아야/EC + 하/VX', '해야 하/XSV + 아야/EC + 하/VX'] and \
        backward_pairs and str(backward_pairs[0].morphs[0]) == '겠/EP':
      sandwich_pairs.append(WordStrMorphsPair(uni_word, morphs[:-1]))
      backward_pairs[0].morphs.insert(0, morphs[-1])
      del morphs[:]
    elif word_morphs_str == '야 아야/EC + 하/VX' and backward_pairs and str(backward_pairs[0].morphs[0]) == '겠/EP':
      sandwich_pairs.append(WordStrMorphsPair(uni_word, morphs[:-1]))
      backward_pairs[0].morphs.insert(0, morphs[-1])
      del morphs[:]
    elif word_morphs_str in ['졌 어/EC + 지/VX + 었/EP', '져 어/EC + 지/VX + 어/EC'] \
        and forward_pairs and forward_pairs[-1].morphs[-1].tag == 'VV':
      forward_pairs[-1].morphs.append(morphs[0])
      sandwich_pairs.append(WordStrMorphsPair(uni_word, morphs[1:]))
      del morphs[:]
    elif word_morphs_str in ['왔 아/EC + 오/VX + 았/EP', '왔 어/EC + 오/VX + 았/EP'] \
        and forward_pairs and forward_pairs[-1].morphs[-1].tag == 'VV':
      forward_pairs[-1].morphs.append(morphs[0])
      sandwich_pairs.append(WordStrMorphsPair(uni_word, morphs[1:]))
      del morphs[:]
    elif uni_word in [u'해졌', u'해봤', u'해했', u'해봐', u'해줬', u'해줘', u'해와'] and len(morphs) == 4:
      sandwich_pairs.append(WordStrMorphsPair(uni_word[0], morphs[:2]))
      sandwich_pairs.append(WordStrMorphsPair(uni_word[1], morphs[2:]))
      del morphs[:]
    elif word_morphs_str  == '봤 아/EC + 보/VX + 았/EP' and forward_pairs and forward_pairs[-1].morphs[-1].tag == 'VV':
      forward_pairs[-1].morphs.append(morphs[0])
      sandwich_pairs.append(WordStrMorphsPair(uni_word, morphs[1:]))
      del morphs[:]
    else:
...
def align(word):
  ...
  try:
    sandwich_pairs = proc_sandwich(forward_pairs, uni_word, morphs, backward_pairs)
